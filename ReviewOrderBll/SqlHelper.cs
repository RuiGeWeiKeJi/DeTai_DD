...
namespace StudentMgr
{
    public static class SqlHelper
    {
        ...
        public static string Connstr
        {
            get
            {
                ...
                if ( string . IsNullOrEmpty ( connstr ) )
                {
                    ...
                    connstr = string . Format ( "Data Source={0};Initial Catalog={1};User Id={2};Password={3};Connect Timeout=60" ,host ,dbname ,user ,password );
                }
            ...
            params SqlParameter [ ] parameters )
        {
            ...
            {
                conn . Open ( );
                int rows= ExecuteNonQuery ( conn ,cmdText ,parameters );
                ...
                using ( SqlCommand cmd = new SqlCommand( ) )
                {
                    try
                    {
                        ...
                        else
                        {
                            try
                            {
                                LogHelperToSql . SaveLog ( SQLSting );
                            }
                            catch ( Exception ex ) { Utility . LogHelper . WriteLog ( ex . Message ); }
                ...
                using ( SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        ...
                        else
                        {
                            try
                            {
                                LogHelperToSql . SaveLog ( SQLString );
                            }
                            catch ( Exception ex ) { Utility . LogHelper . WriteLog ( ex . Message + "\n\r" + ex . StackTrace ); }
        ...
        public static string   UpdateGrid ( )
        {
            ...
            {
                using ( SqlCommand command = new SqlCommand ( "SELECT PRO001 FROM DETPRO WHERE PRO008='P' AND (PRO010='' OR PRO010 IS NULL)" ,connection ) )
                {
                    ...
                    //while ( sdr . Read ( ) )
                    //{
                    //    str = string . Format ( "Id:{0}\tUserId:{1}\tMessage:{2}" ,sdr [ "ID" ] . ToString ( ) ,sdr [ "UserId" ] . ToString ( ) ,sdr [ "Message" ] . ToString ( ) );
                    //}
                    //sdr . Close ( );
                }
            }
